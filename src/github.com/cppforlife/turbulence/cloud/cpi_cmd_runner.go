...
func (r *cpiCmdRunner) Run(context CmdContext, method string, args ...interface{}) (CmdOutput, error) {
	...
	r.logger.Debug(r.logTag,
		...
		exitCode, cmdPath, string(inputBytes), stdout, stderr)
	...
	if err != nil {
		return CmdOutput{}, bosherr.WrapErrorf(err,
			"Unmarshalling external CPI command output: STDOUT: '%s', STDERR: '%s'", stdout, stderr)
	}
